# Makefile for grove-project-tmpl-go (ptg)

BINARY_NAME=ptg
E2E_BINARY_NAME=tend-ptg
BIN_DIR=bin
...
# --- E2E Testing ---
# Build the custom tend binary for grove-project-tmpl-go E2E tests.
test-e2e-build:
	@echo "Building E2E test binary $(E2E_BINARY_NAME)..."
	@go build $(LDFLAGS) -o $(BIN_DIR)/$(E2E_BINARY_NAME) ./tests/e2e

# Run E2E tests. Depends on the main 'ptg' binary and the test runner.
# Pass arguments via ARGS, e.g., make test-e2e ARGS="run -i"
test-e2e: build test-e2e-build
	@echo "Running E2E tests..."
	@PTG_BINARY=$(abspath $(BIN_DIR)/$(BINARY_NAME)) $(BIN_DIR)/$(E2E_BINARY_NAME) run
...
help:
	...
	@echo "  make test-e2e ARGS=...- Run E2E tests (e.g., ARGS=\"run -i ptg-basic-generation\")"